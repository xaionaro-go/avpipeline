...
func main() {
	...
	if outputFormatContext = astiav.AllocFormatContext(); outputFormatContext == nil {
		panic(errors.New("main: output format context is nil"))
	}
						...
						for {
							...
							if stop, err := func() (bool, error) {
								...
								inPkt := packet.BuildInput(
									s.encPkt, s.outputStream, outputFormatContext,
								)
								_, err := output.GetOutputStream(ctx, inPkt)
								if err != nil {
									panic(err)
								}
