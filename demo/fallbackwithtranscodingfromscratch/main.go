...
func main() {
	...
	decoderMainNode := node.NewFromKernel(ctx, decoderMain, processor.DefaultOptionsTranscoder()...)
	...
	decoderFallbackNode := node.NewFromKernel(ctx, decoderFallback, processor.DefaultOptionsTranscoder()...)
	...
	for {
		...
		case <-statusTicker.C:
			...
			inputMainStatsJSON, err := json.Marshal(inputMainStats.Sent.Packets.ToStats())
			assert(ctx, err == nil, err)

			inputFallbackStats := inputFallbackNode.GetCountersPtr()
			inputFallbackStatsJSON, err := json.Marshal(inputFallbackStats.Sent.Packets.ToStats())
			assert(ctx, err == nil, err)
